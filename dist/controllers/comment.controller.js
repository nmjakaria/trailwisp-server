...
export async function getCommentsForTarget(req, res) {
    const filter = { targetId: req.params.targetId }; // ← ফিক্স
    const comments = await Comment.find(filter)
        .populate('userId', 'name image')
        .sort({ createdAt: -1 });
    res.json(comments);
}
export async function getBestComments(req, res) {
    const comments = await Comment.find({ isBestComment: true }).populate('userId', 'name image').limit(6);
    res.json(comments);
}
export async function toggleBestComment(req, res) {
